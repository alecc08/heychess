...
public class ForceGiveRewardCommand extends RewardCommand {
    ...
    protected ForceGiveRewardCommand(String giver, Set<String> receivers, String channel, int coconutCount) {
        super(giver);
    ...
    public static Predicate<SlackRequestDTO> getPredicate() {
        return request -> {
            if (request.getEvent().getText() != null) {
                ...
                if (text.contains(GlobalConfig.COCONUT_EMOJI) && text.contains(TAG_START) && text.toLowerCase().contains("sudo ") && !text.toLowerCase().contains(" gift") && !text.toLowerCase().contains("sudo say") && !text.toLowerCase().contains("sudo steal")) {
                    return true;
                }
            }
            return false;
    ...
    public static RewardCommand build(SlackRequestDTO request) {
        ...
        return new ForceGiveRewardCommand(request.getEvent().getUser(), receivers, request.getEvent().getChannel(), coconutsToGive);
    }
