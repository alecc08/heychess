...
public class StealRewardCommand extends RewardCommand {
    ...
    protected StealRewardCommand(String giver, Set<String> receivers, String channel, int coconutCount) {
        super(giver);
    ...
    public static Predicate<SlackRequestDTO> getPredicate() {
        return request -> {
            if (request.getEvent().getText() != null) {
                ...
                if (text.contains(COCONUT_EMOJI) && text.contains(TAG_START) && text.toLowerCase().contains("steal") && text.toLowerCase().contains("sudo")) {
                    return true;
                }
            }
            return false;
    ...
    public static RewardCommand build(SlackRequestDTO request) {
        ...
        return new StealRewardCommand(request.getEvent().getUser(), receivers, request.getEvent().getChannel(), coconutsToSteal);
    }
    ...
    protected void performAction() {
        ...
        for (String name : receivers) {
            ...
            responseMessage.append("<@").append(name).append("> has lost ").append(coconutCount).append(" coconut").append((coconutCount > 1 ? "s" : "")).append(".");
        }
